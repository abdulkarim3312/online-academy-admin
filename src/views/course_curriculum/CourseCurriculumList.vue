    ...
    <div class="col_12">
      <div class="card no_border">
        <div class="card-header">
          <div class="card-title">Course Curriculum List</div>
        </div>
          ...
          >
            ...
            <template #action="{ row }">
              <div class="d_flex_center table_action_btn">
                <router-link
                  :to="{ name: 'course_curriculum_edit', params: { id: row.id } }"
                  :title="$t('common.edit')"
    ...
    deleteRecord(row) {
      this.$swal({
        ...
        if (result.isConfirmed) {
          this.axios.delete('/curriculums/' + row.id).then(() => {
            this.loadData();
            this.$swal.fire(
