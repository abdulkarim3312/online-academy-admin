    ...
    <div class="col_12">
      <div class="card no_border">
        <div class="card-header">
          <div class="card-title">{{ $t('sidebar.studentMember.studentMember') }}</div>
        </div>
                ...
                <div class="d_flex_start">
                  <div class="dropdown custom_dropdown mr_10">
                    ...
                    <div class="dropdown_menu">
                      <ul>
                        <li><a role="button" @click="changeSituationFilter('all')">{{ $t('common.all') }}</a></li>
                        <li @click="changeSituationFilter('approval')"><a role="button">{{ $t('common.approval') }}</a></li>
                        <li @click="changeSituationFilter('stop')"><a role="button">{{ $t('common.stop') }}</a></li>
                        <li @click="changeSituationFilter('dormancy')"><a role="button">{{ $t('common.dormancy') }}</a></li>
                        <li @click="changeSituationFilter('secession')"><a role="button">{{ $t('common.secession') }}</a></li>
                      </ul>
                    </div>
                  </div>
                </div>
            ...
            <div class="col_12">
              <div class="d_flex_btwn pt_20 pb_20">
                ...
                <div class="d_flex_end col_6 pr_0">
                  <router-link :to="{ name: 'student_add'}" class="btn btn_transparent">{{ $t('common.add_new') }}</router-link>
                </div>
              </div>
            </div>
          </div>
          ...
          <table-component 
            ...
            <template #action="{ row }">
              <div class="d_flex_center table_action_btn">
                <router-link :to="{name: 'student_edit', params:{id: row.id}}" :title="$t('common.edit')" class="btn btn_blue ml_10">
  ...
  data() {
    return {
      ...
      queryParams: {
        page: 1,
        search: '',
        search_columns: 'student_id,name,phone,email',
        sort: 'created_at',
        sort_order: 'desc',
        per_page: 20
      },
      ...
      columns: [
        {
          label: this.$t('sidebar.studentMember.student_id'),
          field: "student_id",
          sortable: true
        },
        {
          label: this.$t('sidebar.studentMember.nickname'),
          field: "name",
          sortable: true
        },
        {
          label: this.$t('sidebar.parentMember.parent_id'),
          field: "parents.parent_id",
          sortable: true
        },
        {
        ...
        {
          label: this.$t('sidebar.studentMember.last_visit_date'),
          field: "last_login",
          sortable: true
        },
        {
          label: this.$t('sidebar.studentMember.student_registration_date'),
          field: "registered_at",
          sortable: true
        },
        {
    ...
    deleteRecord(row) {
      this.$swal({
        ...
        if (result.isConfirmed) {
          this.axios.delete('/students/'+row.id).then(() => {
            this.loadData();
            this.$swal.fire(
