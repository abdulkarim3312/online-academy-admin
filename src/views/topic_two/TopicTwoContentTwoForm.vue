        ...
        <div class="card no_border">
          ...
          <div class="card-body pl_0 pr_0">
            <div class="row">
              ...
              <div class="col_6">
                <div :class="['form-group', {'has_error': errorFor('note_description')}]">
                  <label>Note Description</label>
                    <text-editor-component 
                    :id="'note_description'" 
                    :descriptive="''"
                    :existingData="form.note_description ? form.note_description : ''" 
                    @change="changeEditorValue"
                    />
                    <v-errors :errors="errorFor('note_description')"></v-errors>
                </div>
              </div>
...
export default {
  ...
  created() {
    this.axios.get('/topic-two-content-two-data', this.form)
      .then((response) => {
        if(response.data){
  ...
  methods: {
    ...
    submit() {
      ...
      this.axios.post('/topic-two-content-two', this.form)
        .then(() => {
